...
std::unique_ptr<IndexedSubGraph::MetaDef> FuseConvActivation(const Node& conv, const Node& activation,
  ...
  // we use that op type/domain to match the static xnnpack Conv kernel
  // registration
      ...
      const auto update_value = [&](size_t idx, float& value_to_set) {
        if (num_inputs > idx) {
          ...
          if (arg.Exists()) {
            ...
            // NOTE: we don't support external data in this read. these are scalars so should never be in external
            // files.
