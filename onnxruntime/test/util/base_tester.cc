...
ValidateOutputParams MakeValidateOutputParams(const BaseTester::Data& d) {
  return ValidateOutputParams{d.sort_output_, d.absolute_error_, d.relative_error_};
}

void Check(const BaseTester::Data& expected_data, const Tensor& output_tensor,
           const std::string& provider_type) {
  ValidateOutput(expected_data.def_.Name(), expected_data.data_.Get<Tensor>(), output_tensor, provider_type,
                 MakeValidateOutputParams(expected_data));
}

}  // namespace

//// TODO: move this to test_utils.h so the ValidateOutput handles non-tensor types
// void Check(const BaseTester::Data& expected_data, OrtValue& ort_value,
//            const std::string& provider_type) {
//   CheckDispatch<
...
// #endif
//       TensorSeq>(expected_data.data_.Type(), expected_data, ort_value, provider_type);
// }
...
std::vector<OrtValue> BaseTester::ExecuteModel(Model& model, SessionType& session,
  ...
  auto status = session.Load(sstr, allow_released_onnx_opset_only);
  ...
  for (int i = 0; i < num_run_calls_; ++i) {
    fetches.clear();
    status =
        session.Run(run_options ? *run_options : default_run_options,
...
#d ef ine ASSERT_PROVIDER_STATUS_OK(function) do { \
    Status _tmp_status = function;                 \
    ASSERT_TRUE(_tmp_status.IsOK()) << "provider: " << provider_type << ", error: " << _tmp_status; } while (false)
...
void BaseTester::RunWithConfig(size_t* number_of_pre_packed_weights_counter,
  ...
  ORT_TRY {
    ...
    auto p_model = GetModel();  // !cache_enabled ? BuildGraph({}, model_options) : cached_model_;
...
void BaseTester::AddReferenceOutputs(const std::string& model_path, float abs_error, std::unique_ptr<IExecutionProvider> ep) {
  SessionOptions so;
  so.session_logid = op_;
  so.session_log_verbosity_level = 1;
...
template std::vector<OrtValue> BaseTester::ExecuteModel<training::TrainingSession>(
