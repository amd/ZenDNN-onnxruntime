...
/// Please include provider_test_utils.h and use OpTester or ModelTester.
/// </summary>
class BaseTester {
 public:
 ...
 protected:
  // ctor for testing an individual operator
  explicit BaseTester(const char* op, int opset_version, const std::string& domain, bool verify_output)
  ...
  // ctor for testing an existing model. implement GetModel to return the model to test.
  BaseTester()
      : op_{"existing model"} {
  }
...
void Check(const BaseTester::Data& expected_data, const Tensor& output_tensor, const std::string& provider_type);
