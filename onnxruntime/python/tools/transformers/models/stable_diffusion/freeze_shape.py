...
class ShapeFreezer(OnnxModel):
    ...
    def use_static_input(self, batch_size=1, sequence_length=77, channels=4, height=64, width=64):
        ...
        for input in self.model.graph.input:
            if input.name in ["input_ids"]:  # text_encoder
            ...
            if input.name in ["latent_sample", "sample", "timestep", "encoder_hidden_states"]:  # Unet, Vae Decoder
                dim_proto = input.type.tensor_type.shape.dim[0]
                if dim_proto.HasField("dim_param"):
                    if input.name == "timestep":
                        dim_proto.dim_value = 1
                    elif input.name in ["latent_sample"]:
                        dim_proto.dim_value = batch_size
                    elif input.name in ["sample", "encoder_hidden_states"]:
    ...
    def unet_dummy_inputs(
        self, elem_type, batch_size=1, sequence_length=77, channels=4, height=64, width=64, hidden_size=768
    ):
        """
        Create dummy data for model inputs. If the model has more than 3 inputs, please update this function before running the tool.
        """
        ...
        return {"sample": input_1, "timestep": input_2, "encoder_hidden_states": input_3}

    def vae_decoder_dummy_inputs(self, elem_type, batch_size=1, channels=4, height=64, width=64):
        """
        Create dummy data for vae decoder.
        """
        assert elem_type in [1, 10]  # only float32 and float16 are supported.

        # Create dummy inputs

        input_1 = np.ones((batch_size, channels, height, width), dtype=np.float16)
        if elem_type == 1:  # float32
            input_1 = np.float32(input_1)

        return {"latent_sample": input_1}

    def text_encoder_dummy_inputs(self, batch_size=1, sequence_length=77):
        """
        Create dummy data for text encoder
        """
        input_1 = np.ones((batch_size, sequence_length), dtype=np.int32)
        return {"input_ids": input_1}

    def create_dummy_inputs(self, elem_type, graph):
        if len(graph.input) == 3:  # unet
            return self.unet_dummy_inputs(elem_type)
        if graph.input[0].name == "input_ids":
            return self.text_encoder_dummy_inputs()
        if graph.input[0].name == "latent_sample":
            return self.vae_decoder_dummy_inputs(elem_type)
        raise ValueError(f"graph input does not match unet/vae decoder/text encoder {graph.input}")
    ...
    def shape_optimization(
        ...
        inputs = self.create_dummy_inputs(elem_type, self.model.graph)
...
def set_last_node(onnx_model, last_node_name):
    first_output_name = onnx_model.model.graph.output[0]
