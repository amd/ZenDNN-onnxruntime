...
from onnxruntime.transformers.io_binding_helper import StaticShapeIOBindingHelper
...
class OrtStableDiffusionPipeline(OnnxStableDiffusionPipeline):
    ...
    def prepare_io_binding(self, batch, height, width, encoder_hidden_states):
        ...
        self.model_bindings["unet"] = StaticShapeIOBindingHelper(self.unet.model, io_shape)

        io_shape = {
            "latent_sample": [batch, 4, height // 8, width // 8],
            "sample": [batch, 3, height, width],
        }
        self.model_bindings["vae_decoder"] = StaticShapeIOBindingHelper(self.vae_decoder.model, io_shape)
        ...
        self.model_bindings["text_encoder"] = StaticShapeIOBindingHelper(self.text_encoder.model, io_shape)
    ...
    def __call__(
        ...
        self.prepare_io_binding(effective_batch, height, width, prompt_embeds)

        # get the initial random noise unless the user supplied it
        latents_dtype = prompt_embeds.dtype
        latents_shape = (effective_batch, 4, height // 8, width // 8)
        if latents is None:
        ...
        for i in range(latents.shape[0]):
            self.model_bindings["vae_decoder"].io_ort_value["latent_sample"].update_inplace(latents[i : i + 1])
            self.vae_decoder.model.run_with_iobinding(self.model_bindings["vae_decoder"].io_binding)
            results.append(self.model_bindings["vae_decoder"].io_ort_value["sample"].numpy())
