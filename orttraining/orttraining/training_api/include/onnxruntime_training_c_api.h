...
struct OrtTrainingApi {
  ...
  /** \brief Save the training checkpoint to a ONNX model for inference on disk.
   *
   * This function converts the training checkpoint files and serializes them
   * to a ONNX tensorproto file on disk. This tensorproto file can later be loaded by ORT for inference.
   *
   * \param[in] checkpoint_path Path to the checkpoint directory
   * \param[in] inference_model_path Boolean flag indicating whether or not to save the optimizer states to the checkpoint.
   *
   * \snippet{doc} snippets.dox OrtStatus Return Value
   *
   */
  ORT_API2_STATUS(SaveCheckpointAsInferenceModel, _In_ const ORTCHAR_T* checkpoint_path, _In_ const ORTCHAR_T* inference_model_path);
