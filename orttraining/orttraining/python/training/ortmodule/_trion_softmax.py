...
@onnx_op(op_type="TritonSoftmax", inputs=[PyCustomOpDef.dt_float], outputs=[PyCustomOpDef.dt_float])
def triton_softmax(x):
    return _softmax(torch.from_numpy(x).to("cuda:0")).cpu().numpy()


@register_graph_transformer()
def transform_softmax(graph):
    for node in graph.node:
        if node.op_type == "Softmax":
            node.op_type = "TritonSoftmax"
